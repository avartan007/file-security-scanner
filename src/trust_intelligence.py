...
class TrustIntelligenceGraph:
    """Build a personal threat intelligence network tracking file origins and patterns."""
    ...
    def _init_tables(self):
        """Create tables for tracking file origins, relationships, and trust scores."""
        with self.conn:
            # Files table
            self.conn.execute("""
            ...
            # Sources table (Telegram, downloads, USB, etc.)
            self.conn.execute("""
            ...
            # File relationships (duplicates, variants, metadata)
            self.conn.execute("""
    ...
    def record_file(self, file_hash, filename, source_origin, vt_score=None, risk_level="UNKNOWN"):
        """Record a file with its origin and initial assessment."""
        with self.conn:
            self.conn.execute("""
            INSERT OR REPLACE INTO files 
            (file_hash, filename, source_origin, vt_score, risk_level, last_seen)
            VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            """, (file_hash, filename, source_origin, vt_score, risk_level))
        
        self._log_audit(file_hash, "RECORDED", f"Source: {source_origin}, Risk: {risk_level}")
    
    def record_source(self, source_name, source_type="UNKNOWN"):
        """Register a file source (Telegram channel, download folder, etc.)."""
        with self.conn:
            self.conn.execute("""
    ...
    def link_relationship(self, hash1, hash2, relationship_type, similarity_score=None):
        """Track relationships between files (duplicates, variants, etc.)."""
        with self.conn:
            self.conn.execute("""
            INSERT INTO relationships (file_hash1, file_hash2, relationship_type, similarity_score)
            VALUES (?, ?, ?, ?)
            """, (hash1, hash2, relationship_type, similarity_score))
        
        self._log_audit(hash1, "LINKED", f"Related to {hash2[:8]}... ({relationship_type})")
    
    def update_trust_score(self, file_hash, score):
        """Update trust score for a file (0.0 = untrusted, 1.0 = trusted)."""
        with self.conn:
            self.conn.execute("""
    ...
    def get_file_history(self, file_hash):
        ...
        SELECT filename, source_origin, first_seen, last_seen, vt_score, risk_level, trust_score, notes
    ...
    def get_source_reputation(self, source_name):
        ...
        cursor = self.conn.execute("""
        SELECT source_type, first_observed, file_count, malware_count, trust_rating, notes
    ...
    def get_related_files(self, file_hash):
        ...
        return [{"hash": row[0], "type": row[1], "similarity": row[2]} for row in cursor.fetchall()]
    ...
    def get_audit_trail(self, file_hash):
        ...
        return [{"action": row[0], "timestamp": row[1], "details": row[2]} for row in cursor.fetchall()]
    ...
    def generate_report(self, output_file="trust_report.json"):
        ...
        FROM files WHERE risk_level IN ('MALICIOUS', 'SUSPICIOUS') ORDER BY vt_score DESC LIMIT 20
        """)
